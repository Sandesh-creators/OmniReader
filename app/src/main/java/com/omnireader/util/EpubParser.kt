...
class EpubParser(private val context: Context) {
    ...
    fun parseEpub(uri: Uri): Result<EpubBook> = runCatching {
        ...
        stream.use { input ->
            ...
            // Extract cover using same logic as extractMetadata
            val coverUri = runCatching {
                context.contentResolver.openInputStream(uri)?.use { input ->
                    val coverPath = findCoverImage(opfContent, opfDir, opfInfo.spineItems, entries)
                    coverPath?.let { path ->
                        readSingleBinaryEntry(input, path)?.let { data ->
                            saveCoverToCache(null, data, path.substringAfterLast('.', "png"))
                        }
                    }
                }
            }.getOrNull()
    ...
    fun extractMetadata(uri: Uri, absPath: String? = null): Result<EpubMetadata> = runCatching {
        val stream = context.contentResolver.openInputStream(uri)
            ?: throw IllegalStateException("Cannot open EPUB file")
        ...
        stream.use { input ->
            ...
            val cachedCover = absPath?.let { loadCoverFromCache(it) }
            ...
            val coverUri = when {
                ...
                coverPath != null -> {
                    runCatching {
                        context.contentResolver.openInputStream(uri)?.use { input ->
                            readSingleBinaryEntry(input, coverPath)?.let { data ->
                                saveCoverToCache(absPath, data, coverPath.substringAfterLast('.', "png"))
                            }
                        }
                    }.getOrNull()
                }
            ...
            EpubMetadata(
                title = metadata["title"] ?: extractTitleFromFilename(uri),
    ...
    private fun saveCoverToCache(absPath: String?, data: ByteArray, ext: String): Uri {
        // Skip SVG - BitmapFactory can't decode it
        if (ext.lowercase() == "svg") {
            throw IllegalStateException("SVG covers not supported")
        }
        // Verify the image is decodable before caching
        val options = BitmapFactory.Options().apply { inJustDecodeBounds = true }
        BitmapFactory.decodeByteArray(data, 0, data.size, options)
        if (options.outWidth <= 0 || options.outHeight <= 0) {
            throw IllegalStateException("Cover image not decodable")
        }
        val fileName = coverCacheName(absPath, ext)
        val tempFile = context.cacheDir.resolve(fileName)
        if (!tempFile.exists()) {
    ...
    private fun loadCoverFromCache(absPath: String): Uri? {
        val knownExts = listOf("jpg", "jpeg", "png", "gif", "webp")
        for (ext in knownExts) {
            val f = context.cacheDir.resolve(coverCacheName(absPath, ext))
            if (f.exists()) return Uri.fromFile(f)
        }
        return null
    }

    private fun coverCacheName(absPath: String?, ext: String): String =
        "cover_${(absPath ?: "no_path").hashCode()}.$ext"
    ...
    private fun findCoverImage(
        ...
        // 3. manifest item whose id contains "cover"
        ...
        // 4. manifest item whose href contains "cover"
        ...
        // 5. first <img> or SVG <image> inside first spine HTML document
        for (href in spineItems) {
            ...
            // Try SVG <image> with xlink:href or href
        ...
        // 6. first image referenced in the manifest
...
private fun resolvePath(baseDir: String, href: String): String {
    ...
    // Resolve a spine href (which may be URL-encoded, case-differing, or a relative
    // "./x" path) to the actual zip entry key. Without this, any non-exact match
    // silently drops that chapter from the reader's chapter list.
    ...
    private fun normalizeTocPath(path: String): String {
        ...
        return if ('%' in slashed) {
            java.net.URLDecoder.decode(slashed, java.nio.charset.StandardCharsets.UTF_8)
        } else {
            slashed
        }
    }
    ...
    private fun extractTitleFromFilename(uri: Uri): String {
        val path = uri.lastPathSegment ?: return "Unknown Title"
